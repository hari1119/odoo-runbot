...
class CpSmsConfiguration(models.Model):
    ...
    def sms_config_data(self, **kw):
        """Geting Mobile no from sms configuration : mail_type,model_name,mail_name key value is must"""
        ...
        if message_type and action_name: 
            if message_type == 'sms':
                sms_form_ids = self.env['cp.sms.configuration'].search(
                    [('active', '=', True),('status', '=', 'active'),('name','=',action_name),('message_type','=',message_type)])
    ...
    def retrieve_dashboard(self):
        ...
        #counts
        cm_master = self.env['cp.sms.configuration']
