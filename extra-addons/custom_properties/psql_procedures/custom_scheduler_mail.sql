-- FUNCTION: public.custom_scheduler_mail(character)

-- DROP FUNCTION IF EXISTS public.custom_scheduler_mail(character);

CREATE OR REPLACE FUNCTION public.custom_scheduler_mail(
	v_trans_source character)
    RETURNS text
    LANGUAGE 'plpgsql'
    COST 100
...
v_net_amt = 0;
			...
			* {-webkit-font-smoothing: antialiased;}
			...
			#customers tr:nth-child(even) {background-color:none!important;}
			...
			<tr>
			<td width="100%">
			<div class="webkit" style="max-width:1400px;Margin:0 auto;">
			<table class="outer" align="center" cellpadding="0" cellspacing="0" border="0" style="border-spacing:0;Margin:0 auto;width:100%;max-width:1200px;">
			<tr>
			...
			v_table_heading= v_table_heading || '<table id="customers" style="border:none;">
				...
				<tr style="border-left:1px solid #007AFF;border-right:1px solid #007AFF;"><td colspan="26" style="border:none;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <span>The following transaction has been approved.</span></td></tr></table>';
			...
			<tr>
                ...
                <th colspan="2"><b><center>Ref No</center></b></th>
                <th colspan="2"><b><center>Partner</center></b></th>
                <th colspan="2"><b><center>Division</center></b></th>
                <th colspan="2"><b><center>Department</center></b></th>
				<th colspan="2"><b><center>Net Amount</center></b></th>
			...
			select  trans.id,trans.name,div.name as div,dep.name as dep,
			partner.name as partner,trans.line_count,trans.net_amt from ct_transaction trans
			left join ct_transaction_line line on line.header_id = trans.id
			...
			where trans.status = 'approved' and trans.ap_rej_date::date = Now()::date 
			group by trans.id,trans.name,div.name,dep.name,partner.name,trans.line_count
			order by trans.name,partner.name,div.name,dep.name;			
			
			LOOP                 
		  		FETCH cursor_temp INTO v_trans_id,v_trans_name,v_division,v_department,v_supplier,v_line_count,v_trans_tot_amt;
			...
			if(v_line_count is null) then
				v_line_count=0;
			end if;

			if(v_trans_tot_amt is null) then
				v_trans_tot_amt=0;
			end if;
			
			v_data=v_data ||
				'<tr>
				<td colspan="2" rowspan="'||v_line_count||'"><center>'||v_sl_no||'</center></td>
                <td colspan="2" rowspan="'||v_line_count||'">'||v_trans_name||'</td>
				<td colspan="2" rowspan="'||v_line_count||'">'||v_supplier||'</td>
				<td colspan="2" rowspan="'||v_line_count||'">'||v_division||'</td>
				<td colspan="2" rowspan="'||v_line_count||'">'||v_department||'</td>
				<td colspan="2" rowspan="'||v_line_count||'" align="right">'||trim(TO_CHAR((v_trans_tot_amt::numeric)::float, '99G99G99G99G99G99G990D99'))||'</td>';
			
			v_net_amt=v_net_amt+v_trans_tot_amt;
			Open cursor_temp123 FOR
            
            select  line.description,brand.name,uom.name->>'en_US' as UOM,
			round(line.qty::numeric,3) as qty,
			...
			line.line_tot_amt::numeric as line_tot_amt from ct_transaction_line line
			left join  ct_transaction trans on trans.id = line.header_id 
			left join uom_uom uom on uom.id = line.uom_id
			left join cm_master brand on brand.id = line.brand_id
			where trans.status = 'approved'
			and line.header_id = v_trans_id;
            LOOP                 
                  FETCH cursor_temp123 INTO v_description,v_brand,v_uom,v_qty,v_disc_amt,v_tax_amt,v_unitprice,v_line_tot_amt;
			...
			if(v_line_tot_amt is null) then
				...
				v_data=v_data ||'<td colspan="2">'||v_description||'</td>
                ...
                <td colspan="2" align="right">'||trim(TO_CHAR((v_line_tot_amt::numeric)::float, '99G99G99G99G99G99G990D99'))||'</td><tr>';
			END LOOP;
	...
	v_data=v_data || '</tr>
                ...
                <td colspan="10" align="right"><b>Grand Total(<font style="font-size:13px;">INR</font>)</b></td>
                <td colspan="2" align="right"><b>'||trim(TO_CHAR((round(v_net_amt::numeric,2))::float, '99G99G99G99G99G99G990D99'))||'</b></td>
				<td colspan="16"></td>
            </tr>';
...
ALTER FUNCTION public.custom_scheduler_mail(character)
    OWNER TO odoo;
