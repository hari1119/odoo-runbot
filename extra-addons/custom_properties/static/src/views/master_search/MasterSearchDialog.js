...
export class MasterSearchDialog extends Component {
    ...
    // Compute the state based on the provided options.
        ...
        const result = await this.rpc("/master/search", {
          search : {'query': query, 'options': this.state.options},
        });
        ...
        if (result.length != 0 && this.state.options == "only-name") {
        ...
        else if (result.length != 0 && this.state.options == "global") {
          for (const record of result) {
            var temp_items = [];
            for (const rec of record) {
                temp_items.push({
    ...
    get emptyText() {
        if (this.state.query != "" && this.state.result == null) {
            return this.props.emptyText ?? _t("No Matches Found");
         }
    }
